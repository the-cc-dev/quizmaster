...
class QuizMaster_Helper_ExportXml
{
    ...
    public function export($ids)
    {
        ...
        $root = $dom->createElement('quizMaster');

        $dom->appendChild($root);

        $header = $dom->createElement('header');
        $header->setAttribute('version', QUIZMASTER_VERSION);
    ...
    private function getQuizElement($dom, $quiz, $forms)
    {
        ...
        if ($quiz->getAdminEmail() !== null) {
            /* @var $adminEmail QuizMaster_Model_Email */
    ...
    /**
     * @param DOMDocument $dom
     * @param QuizMaster_Model_Question $question
     * @return DOMDocument
     */
    private function createQuestionElement($dom, $question)
    {
        ...
        if (is_array($answerData)) {
            foreach ($answerData as $answer) {
                /** @var QuizMaster_Model_AnswerTypes $answer */
