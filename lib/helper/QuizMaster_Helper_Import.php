...
class QuizMaster_Helper_Import
{
    ...
    public function setImportFileUpload($file)
    {
        if (!is_uploaded_file($file['tmp_name'])) {
            $this->setError(__('File was not uploaded', 'quizmaster'));
    ...
    private function checkCode()
    {
        ...
        if ($c !== 'WPQ') {
            $this->setError(__('File have wrong format', 'quizmaster'));
    ...
    public function getImportData()
    {
        ...
        if ($this->_content === null) {
            $this->setError(__('File cannot be processed', 'quizmaster'));
    ...
    private function importData($o, $ids = false, $version = '1')
    {
        ...
        $categoryArrayQuiz = $categoryMapper->getCategoryArrayForImport(QuizMaster_Model_Category::CATEGORY_TYPE_QUIZ);

        foreach ($o['master'] as $master) {
            /** @var QuizMaster_Model_Quiz $master */

            if (get_class($master) !== 'QuizMaster_Model_Quiz') {
                continue;
            }
            ...
            if ($version == 3) {
                if ($master->isQuestionOnSinglePage()) {
                    $master->setQuizModus(QuizMaster_Model_Quiz::QUIZ_MODUS_SINGLE);
                } else {
                    if ($master->isCheckAnswer()) {
                        $master->setQuizModus(QuizMaster_Model_Quiz::QUIZ_MODUS_CHECK);
                    } else {
                        if ($master->isBackButton()) {
                            $master->setQuizModus(QuizMaster_Model_Quiz::QUIZ_MODUS_BACK_BUTTON);
                        } else {
                            $master->setQuizModus(QuizMaster_Model_Quiz::QUIZ_MODUS_NORMAL);
                        }
                    }
                }
            }
            ...
            if (trim($master->getCategoryName()) != '') {
                if (isset($categoryArrayQuiz[strtolower($master->getCategoryName())])) {
                    ...
                    $categoryModel = new QuizMaster_Model_Category();
                    $categoryModel->setCategoryName($master->getCategoryName());
                    $categoryModel->setType(QuizMaster_Model_Category::CATEGORY_TYPE_QUIZ);
            ...
            if (isset($o['forms']) && isset($o['forms'][$oldId])) {
                foreach ($o['forms'][$oldId] as $form) {
                    /** @var QuizMaster_Model_Form $form * */
            ...
            foreach ($o['question'][$oldId] as $question) {
                ...
                if (get_class($question) !== 'QuizMaster_Model_Question') {
                    continue;
                }
