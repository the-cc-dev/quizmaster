...
class QuizMaster_Helper_ImportXml
{
    ...
    private function checkCode()
    {
        ...
        if ($xml === false) {
            $this->_error = __('XML could not be loaded.', 'quizmaster');
    ...
    public function saveImport($ids)
    {
        ...
        foreach ($data['master'] as $quiz) {
            ...
            if (get_class($quiz) !== 'QuizMaster_Model_Quiz') {
                continue;
            }
     ...
     * @return QuizMaster_Model_Form
     */
    private function createFormModel($xml)
    {
        $form = new QuizMaster_Model_Form();
    ...
    /**
     * @param SimpleXMLElement $xml
     * @return null|QuizMaster_Model_Quiz
     */
    private function createQuizModel($xml)
    {
        $model = new QuizMaster_Model_Quiz();
        ...
        if (isset($xml->adminEmail)) {
            $adminEmail = new QuizMaster_Model_Email();
        ...
        if (isset($xml->userEmail)) {
            $userEmail = new QuizMaster_Model_Email();
     ...
     * @return NULL|QuizMaster_Model_Question
     */
    private function createQuestionModel($xml)
    {
        $model = new QuizMaster_Model_Question();
        ...
        if (isset($xml->answers)) {
            foreach ($xml->answers->answer as $answer) {
                $answerModel = new QuizMaster_Model_AnswerTypes();
