...
class QuizMaster_Model_CategoryMapper extends QuizMaster_Model_Mapper
{

    /**
     * @param string $type
     *
     * @return QuizMaster_Model_Category[]
     */
    public function fetchAll($type = QuizMaster_Model_Category::CATEGORY_TYPE_QUESTION)
    {
        $type = $type == QuizMaster_Model_Category::CATEGORY_TYPE_QUESTION ? $type : QuizMaster_Model_Category::CATEGORY_TYPE_QUIZ;
        ...
        foreach ($results as $row) {
            $r[] = new QuizMaster_Model_Category($row);
        }
    ...
    public function fetchByQuiz($quizId)
    {
        ...
        $results = $this->_wpdb->get_results($this->_wpdb->prepare('
			...
			ORDER BY
       			c.category_name
		', QuizMaster_Model_Category::CATEGORY_TYPE_QUESTION, $quizId), ARRAY_A);
    ...
    public function save(QuizMaster_Model_Category $category)
    {
    ...
    public function getCategoryArrayForImport($type = QuizMaster_Model_Category::CATEGORY_TYPE_QUESTION)
    {
        $type = $type == QuizMaster_Model_Category::CATEGORY_TYPE_QUESTION ? $type : QuizMaster_Model_Category::CATEGORY_TYPE_QUIZ;
