...
class QuizMaster_Controller_Toplist extends QuizMaster_Controller_Controller
{
    ...
    private function showAdminToplist($quizId)
    {
        ...
        $view = new QuizMaster_View_AdminToplist();
    ...
    public function getAddToplist(QuizMaster_Model_Quiz $quiz)
    {
        ...
        $data = array(
            'userId' => $userId,
            'token' => wp_create_nonce('quizMaster_toplist'),
    ...
    private function handleAddInToplist(QuizMaster_Model_Quiz $quiz)
    {
        if (!wp_verify_nonce($this->_post['token'], 'quizMaster_toplist')) {
            return array('text' => __('An error has occurred.', 'quizmaster'), 'clear' => true);
        }
        ...
        $quizMapper = new QuizMaster_Model_QuizMapper();
        $toplistMapper = new QuizMaster_Model_ToplistMapper();
        ...
        if ($userId > 0) {
            if ($toplistMapper->countUser($quizId, $userId, $clearTime)) {
                return array('text' => __('You can not enter again.', 'quizmaster'), 'clear' => true);
            }
            ...
            if (empty($name) || empty($email) || filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
                return array('text' => __('No name or e-mail entered.', 'quizmaster'), 'clear' => false);
            }

            if (strlen($name) > 15) {
                return array('text' => __('Your name can not exceed 15 characters.', 'quizmaster'), 'clear' => false);
            }
            ...
            if ($quiz->isToplistDataCaptcha()) {
                ...
                if ($captcha->isSupported()) {
                    if (!$captcha->check($prefix, $captchaAnswer)) {
                        return array('text' => __('You entered wrong captcha code.', 'quizmaster'), 'clear' => false);
                    }
                }
            }
        }

        $toplist = new QuizMaster_Model_Toplist();
    ...
    {
        switch ($type) {
            case QuizMaster_Model_Quiz::QUIZ_TOPLIST_TYPE_ALL:
                return true;
            case QuizMaster_Model_Quiz::QUIZ_TOPLIST_TYPE_ONLY_ANONYM:
                return $userId == 0;
            case QuizMaster_Model_Quiz::QUIZ_TOPLIST_TYPE_ONLY_USER:
                return $userId > 0;
        }
    ...
    public static function ajaxAddInToplist($data)
    {
        ...
        $ctn = new QuizMaster_Controller_Toplist();
        ...
        if ($quiz->isToplistActivated() && $quiz->isToplistDataCaptcha() && get_current_user_id() == 0) {
            ...
            if ($captcha->isSupported()) {
                ...
                if ($r !== true) {
                    $r['captcha']['img'] = QUIZMASTER_CAPTCHA_URL . '/' . $captcha->createImage();
        ...
        if ($r === true) {
            $r = array('text' => __('You signed up successfully.', 'quizmaster'), 'clear' => true);
        }
